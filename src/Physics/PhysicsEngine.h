...
namespace Physics
{
    ...
    struct Collider
    {
        ...
        : boundingBox(AABB(_bottomLeft, _topRight))
        , isActive(true)
        , onCollisionCallback(_onCollisionCallback)
        {}

        Collider(AABB _boundingBox, std::function<void(const IGameObject&, const ECollisionDirection)> _onCollisionCallback = {})
    ...
    public:
        ...
        static void addDynamicObject(std::shared_ptr<IGameObject> pGameObject);
