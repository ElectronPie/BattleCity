...
, m_menuSprite(std::make_pair(ResourceManager::getSprite("menu"), glm::vec2(11 * BLOCK_SIZE, STARTSCREEN_HEIGHT - MENU_HEIGHT - 5 * BLOCK_SIZE - startScreenDescription.size() * BLOCK_SIZE)))
...
, m_tankSpriteAnimator(m_tankSprite.first)
{
    ...
    auto bottomOffsetPixels = STARTSCREEN_HEIGHT - 4 * BLOCK_SIZE;

    unsigned int currentBottomOffset = bottomOffsetPixels;
...
{
    for (const auto& currentElemPair : m_sprites)
    {
        if (currentElemPair.first)
        {
            currentElemPair.first->render(currentElemPair.second, glm::vec2(BLOCK_SIZE), 0.f);
        }
    }

    m_menuSprite.first->render(m_menuSprite.second, glm::vec2(MENU_WIDTH, MENU_HEIGHT), 0.f);
    m_tankSprite.first->render(glm::vec2(m_tankSprite.second.x, m_tankSprite.second.y - 2 * BLOCK_SIZE * m_currentMenuSelection), glm::vec2(TANK_SIZE), 0.f, 0.f, m_tankSpriteAnimator.getCurrentFrame());
}
