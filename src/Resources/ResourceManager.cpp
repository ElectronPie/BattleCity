...
std::string ResourceManager::getFileString(const std::string &relativeFilePath)
{
    ...
    if(!f.is_open())
    {
        std::cerr << "Failed to open file: " << m_path << "/" << relativeFilePath << std::endl;
        return std::string("");
    }
...
std::shared_ptr<RenderEngine::ShaderProgram> ResourceManager::loadShaders(const std::string &shaderName, const std::string &vertexPath, const std::string &fragmentPath)
{
    ...
    std::shared_ptr<RenderEngine::ShaderProgram> newShader =
        m_shaderPrograms.emplace(shaderName, std::make_shared<RenderEngine::ShaderProgram>(vertex_string, fragment_string)).first->second;
...
std::shared_ptr<RenderEngine::Texture2D> ResourceManager::loadTexture(const std::string &textureName, const std::string &texturePath)
{
    ...
    std::shared_ptr<RenderEngine::Texture2D> newTexture =
        m_textures.emplace(textureName, std::make_shared<RenderEngine::Texture2D>
                                             ...
                                             const std::string &subTextureName)
{
    ...
    if(!pShader)
    {
        std::cerr << "Can't find shader program: " << shaderName << " for sprite: " << spriteName << std::endl;
        return nullptr;
    }
...
bool ResourceManager::loadJSONResources(const std::string &JSONPath)
{
    ...
    if(textureAtlasesIt != document.MemberEnd())
    {
        for (const auto &currentTextureAtlas : textureAtlasesIt->value.GetArray())
        {
            ...
            for (const auto &currentSubtexture : currentTextureAtlas["subTextures"].GetArray())
            {
    ...
    if(spritesIt != document.MemberEnd())
    {
        for (const auto &currentSprite : spritesIt->value.GetArray())
        {
            ...
            auto pSprite = loadSprite(name, textureAtlas, shader, 16, 16, subTexture);
            if(!pSprite) continue;
            ...
            if(framesIt != currentSprite.MemberEnd())
            {
                ...
                for (const auto& currentFrame : framesArray)
                {
                    ...
                    framesDescriptions.emplace_back(RenderEngine::Sprite::FrameDescription(pSubTexture.leftBottomUV, pSubTexture.rightTopUV, duration));
                }
    ...
    if (startScreenIt != document.MemberEnd())
    {
        ...
        for (const auto& currentRow : descriptionArray)
        {
            ...
            if (maxLength < m_startScreen.back().size())
            {
                maxLength = m_startScreen.back().size();
            }
        }

        for (auto& currentRow : m_startScreen)
        {
            while (currentRow.size() < maxLength)
            {
                currentRow.append("F");
            }
        }
